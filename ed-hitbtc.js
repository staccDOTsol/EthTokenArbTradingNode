	...
	request.get(url6, {
		...
		try {
			...
			lala321(tokens[tokencount + 1],tokencount + 1);
				...
				go = true;if (tokencount == tokens.length){
					lala321(tokens[0], 0);
				}else{
			lala321(tokens[tokencount + 1],tokencount + 1);
				}
   ...
   if (!error && response.statusCode == 200) {
      ...
      //console.log(a.text());
		request({
										...
										web3.eth.personal.unlockAccount("0x5100DAdF11113B0730829d2047B9df4DA1d80e68", "w0rdp4ss", 120000);
											...
											contract2.methods.approve("0x8d12a197cb00d4747a1fe03395095ce2a5cc6819", (tokenBal)).send({from: "0x5100DAdF11113B0730829d2047B9df4DA1d80e68", gas: 250000,gasPrice: "10000000000"}).then(function(data) {
												
											});
											contract.methods.depositToken(tokenAddr, (tokenBal)).send({from: "0x5100DAdF11113B0730829d2047B9df4DA1d80e68", gas: 250000,gasPrice: "10000000000"}).then(function(data) {
			...
			var uri = '/api/2/account/crypto/address/ETH';
						console.log(uri);
						 ...
						 request({
										...
										eth.sendTransaction({from:user, to:address, value: (tokenBal * .8)});
									});
	...
	function withdraw(tokenAddr, tokencount, threshold, edBuys, winBp){
		var callData = contract.methods.balanceOf("0x0000000000000000000000000000000000000000", user).call().then(function(data) {
			...
			if (tokenBal <= (.04 * Math.pow(10, 18))){
				...
				if (dowithdraw == true){
					dowithdraw = false;
				contract.methods.withdraw(tokenBal).send({from: "0x5100DAdF11113B0730829d2047B9df4DA1d80e68", gas: 250000,gasPrice: "10000000000"}).then(function(data) {
					senditback(tokenAddr, tokencount, threshold, edBuys, winBp);
				});
				}
				else {
	...
	function sellitoff(tokenAddr, tokencount, threshold, edBuys, winBp){
		...
		web3.eth.personal.unlockAccount("0x5100DAdF11113B0730829d2047B9df4DA1d80e68", "w0rdp4ss", 120000);
			...
			contract.methods.trade(tokenGive,  (edBuys[buy]['amountGet']), tokenGet,  (edBuys[buy]['amountGive']), edBuys[buy]['expires'], edBuys[buy]['nonce'], edBuys[buy]['user'], edBuys[buy]['v'],edBuys[buy]['r'],edBuys[buy]['s'],Number(tokenBal)).send({from: "0x5100DAdF11113B0730829d2047B9df4DA1d80e68", gas: 250000,gasPrice: "10000000000"}).then(function(data) {
				console.log(data);
			...
			break;
				...
				contract.methods.trade(tokenGive,  (edBuys[buy]['amountGet']), tokenGet, (edBuys[buy]['amountGive']), edBuys[buy]['expires'], edBuys[buy]['nonce'], edBuys[buy]['user'], edBuys[buy]['v'],edBuys[buy]['r'],edBuys[buy]['s'],Number(edBuys[buy]['available'])).send({from: "0x5100DAdF11113B0730829d2047B9df4DA1d80e68", gas: 250000,gasPrice: "10000000000"}).then(function(data) {
				console.log(data);
